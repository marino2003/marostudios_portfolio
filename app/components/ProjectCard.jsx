  ...
  return (
    <Link href={`/projects/${project.slug}`} className={`bg-[#1C1C1C] rounded-3xl overflow-hidden block ${fullWidth ? 'w-full' : 'w-full md:w-[calc(50%-1rem)]'} group/card`} style={{ contain: 'layout style' }}>
      ...
      {project.featuredImage && (
        <div className="relative w-full h-48 sm:h-64 md:h-80 lg:h-[600px] rounded-3xl overflow-hidden">
          <img 
      ...
      <div className="p-6 sm:p-8">
        <div className="flex flex-col md:flex-row justify-between gap-6">
          ...
          <div className="flex-1">
            <h3 className="text-xl sm:text-2xl md:text-3xl font-regular text-white mb-3 font-inter">
              {project.title}
            </h3>
            <div className="flex flex-wrap gap-2">
              {/* Categorie tag - show on all devices */}
              {project.categorie && project.categorie.length > 0 && (
                <span className="px-3 py-1 sm:px-4 sm:py-2 border border-white text-white rounded-full text-sm sm:text-base font-inter">
                  {project.categorie[0]}
                </span>
              )}
